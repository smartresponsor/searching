...
/**
 * Defines the search backend query builder responsibility within the Searching component runtime and its typed boundaries.
 */
final class SearchBackendQueryBuilder implements SearchBackendQueryBuilderInterface
{
    ...
    public function build(SearchQuery $query, SearchProviderConfiguration $configuration): SearchBackendQuery
    {
        ...
        $fieldWeights = null !== $tuning ? $tuning->fieldWeights : [];
        ...
        if ('' === $queryText) {
            ...
            $must[] = [
                'multi_match' => [
                    'query' => $queryText,
                    'fields' => $this->searchFields($configuration, $fieldWeights),
                    'type' => 'best_fields',
